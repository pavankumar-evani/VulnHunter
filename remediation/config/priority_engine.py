...
def compute_priority(finding, rules):
    ...
    score += keyword_score
    if matched_keyword:
        reasons.append(f"asset name matches '{matched_keyword}' (+{keyword_score})")

    asset_type = asset.get("type", "")
    type_score = rules["asset_type_weights"].get(asset_type, 0)
    score += type_score
    if type_score:
        reasons.append(f"asset type {asset_type} (+{type_score})")
...
def compute_sla(finding, priority, rules, as_of=None):
    """Returns {due_date, days_remaining, breached} based on first_seen + the SLA
    window for this finding's priority tier."""
    as_of = as_of or datetime.date.today()
    sla_days = rules["sla_days"].get(priority, 90)

    first_seen_str = finding.get("first_seen")
    if not first_seen_str:
        return {"due_date": None, "days_remaining": None, "breached": None}
...
def score_findings(findings, rules=None, as_of=None):
    ...
    for f in findings:
        ...
        sla_result = compute_sla(f, priority_result["priority"], rules, as_of=as_of)
