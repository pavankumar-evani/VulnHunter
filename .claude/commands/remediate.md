...
5. If `--generate` was passed (or the user confirms after seeing the plan): split the plan
   into windows-server, unix-server, iot-ot-device, and application findings, delegate to
   **remediation-fixer-windows**, **remediation-fixer-unix**, **remediation-fixer-ot**,
   and **remediation-fixer-application** respectively (all four can run independently of
   each other). Report back which playbooks/recommendations/plans were generated - and
   for OT specifically, restate plainly that its output is a recommendation for IT
   security + OT/plant operations to act on, never a change already made; for
   application/SCA findings, restate that the output is a plan for a developer to
   execute through their own normal PR process, never an edit already made to a
   manifest file.
