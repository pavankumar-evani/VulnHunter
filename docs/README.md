...
| [GOING_LIVE.md](GOING_LIVE.md) | The operational checklist for actually connecting a real account — exact credentials needed per connector, exact commands/steps, and which connectors are ready today (ServiceNow/Jira/Splunk) versus which need a script run outside the dashboard (Tenable/Armis/CrowdStrike/Infoblox/Axonius). |
